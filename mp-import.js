...
(function(){
  ...
  function mpDirection(text,amount){
    ...
    const out=["retiro","withdrawal","debito","debit","gasto","compra","purchase","envio de dinero","transfer sent","money out","salida","pago enviado","payment sent","cargo","fee","comision","charge"];
    const inc=["ingreso","credito","credit","cobro","venta","sale","received","recibido","acredit","money in","entrada","refund received","devolucion recibida"];
  ...
  function importMercadoPagoCSV(text,fileName){
    ...
    const headers=rows[0].map(normalizeMPHeader);
    const dateAliases=["date","fecha","release_date","transaction_date","date_created","creation_date","approved_date","operation_date","movement_date"];
    const descAliases=["description","descripcion","detalle","detail","concept","concepto","transaction_detail","operation_description","reference","motivo"];
    const typeAliases=["transaction_type","movement_type","operation_type","type","tipo","transaction_status","source_type"];
    const idAliases=["source_id","transaction_id","operation_id","payment_id","id","external_reference","reference_id"];
    const amountAliases=["transaction_amount","amount","importe","monto","settlement_net_amount","net_amount","total_amount","value"];
    ...
    for(let r=1;r<rows.length;r++){
      const vals=rows[r],obj={};headers.forEach((h,i)=>{if(h)obj[h]=vals[i]??""});
      ...
      const base=[date,amount.toFixed(2),normalizeMPHeader(desc),normalizeMPHeader(typeText),sourceId].join("|");
      ...
      const m=ensureMonth(month),common={id:uid(),amount,date,reason:desc||"Mercado Pago",created:Date.now()+r,mpKey,source:"Mercado Pago CSV",mpFile:fileName||""};
      if(direction==="income"){m.incomes.push(common);incomes++}
      else{m.expenses.push({...common,category:"Mercado Pago"});expenses++}
      existing.add(mpKey);touched.add(month);imported++;
    }
    if(imported){save();renderAll()}
    return {imported,incomes,expenses,duplicates,skipped,months:[...touched].sort()};
  }
  ...
  // Reemplaza los nodos para quitar el listener provisional de v22.7.
